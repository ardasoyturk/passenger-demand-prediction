...
from scripts.shared.duckdb_utils import quote_ident, sql_string, table_columns
from scripts.shared.serialization import write_json
from scripts.stop_addition.common import (
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Build leakage-safe training rows for the stop-addition proposed-route "
            "demand model. Each row is a real historical trip on a discovered "
            "one-stop variant route. Historical features use dates strictly before "
            "the target trip date."
        )
    )
    parser.add_argument("--db", default="analysis.duckdb")
    parser.add_argument(
        "--pair-features",
        default="results/stop_addition/one_stop_route_pair_geographic_features.csv",
        help="Output of analyse_route_pair_geography.py",
    )
    parser.add_argument(
        "--similarity-evidence",
        default="results/stop_addition/route_pair_similarity_evidence.csv",
        help="Static route-similarity output of analyse_route_pair_geography.py",
    )
    parser.add_argument("--output-dir", default="results/stop_addition")
    parser.add_argument("--trip-table", default=TRIP_TABLE_DEFAULT)
    parser.add_argument(
        "--min-similarity",
        type=float,
        default=0.60,
        help="Ignore similar-route candidates below this static similarity score.",
    )
    parser.add_argument(
        "--top-k-similar",
        type=int,
        default=10,
        help="Maximum similar routes retained per pair.",
    )
    parser.add_argument("--train-end", default="2024-12-31")
    parser.add_argument("--validation-end", default="2025-06-30")
    parser.add_argument("--test-end", default="2025-12-31")
    parser.add_argument(
        "--sample-rows",
        type=int,
        default=2000,
        help="Number of rows exported to the human-readable sample CSV.",
    )
...
def recommended_feature_metadata(con: duckdb.DuckDBPyConnection) -> dict[str, object]:
    ...
    recommended = [column for column in columns if column not in FEATURE_METADATA_EXCLUDED]
    ...
    return {
        ...
        "metadata_columns_not_for_model": sorted(FEATURE_METADATA_EXCLUDED - {"target_passenger_count", "data_split"}),
        "notes": [
