...
The `/chat` page uses the OpenAI-compatible relay by default. Configure its
browser-safe settings—provider, model, LLM base URL, and MCP endpoint—in
...
server-side; the browser never receives it. To use this alternative, set
`provider` to `'gateway'` in `src/pages/Chat/config.ts`.

To use a different OpenAI-compatible endpoint, keep `provider` set to
`'openai-compatible'` and change `model` and `openAICompatibleBaseUrl` in
`src/pages/Chat/config.ts`, then set
